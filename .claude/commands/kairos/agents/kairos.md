...
activation-instructions:
  ...
  - STEP 6.1: |
      ...
      2. Mensagens iniciadas por `*` são comandos.
      3. Mensagens sem `*` ainda passam por REQUEST-RESOLUTION. Se houver match razoável, execute o comando correspondente.
      4. Se não houver match de comando, trate como conversa, planejamento, esclarecimento ou pedido ao @kairos.
      5. Se a intenção parecer ser para o runtime/base fora do Kairos, peça confirmação antes de sair do modo @kairos.
  - STEP 6.5: |
...
push_system:
  ...
  push_steps:
    ...
    - modo_contribuidor: "Passo 0c — prompt de bump pendente quando há story type:kairos-core In Review (scope=framework), *push invoca *version inline se 's'. 'Modo contribuidor' = quem contribui upstream com o Kairos público; para uso local/fork, bump é opcional."
