...
- **5.2** — CLI `npx install kairos` / `npx kairos update` (greenfield + brownfield)
- **5.3** — Publicação de `@kairos/core` no npm
- **5.4** — Squad-scaffolding dinâmico (gerar personas em `.claude/commands/` a partir de `squads/{squad}/agents/*.yaml`)
